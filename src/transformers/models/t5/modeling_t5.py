...
class T5SdpaAttention(T5Attention):
    def forward(
        ...
        if output_attentions or layer_head_mask is not None:
            ...
            logger.warning_once(
                "T5Model is using BartSdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True` or `layer_head_mask` not None. Falling back to the manual attention"
        ...
        # Mask heads if we want to TODO: look if we can still do this with sdpa, should actually be possible
        # if layer_head_mask is not None:
        #     attn_weights = attn_weights * layer_head_mask
